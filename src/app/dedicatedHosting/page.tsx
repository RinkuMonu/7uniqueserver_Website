...
const serviceSections = [
  ...
  <div className="container">
    <h2 className="text-center mb-5 fw-bold">Additional Dedicated Hosting Services</h2>
    <div className="row g-4">
      {serviceSections.map((section, idx) => (
        <div className="col-md-3" key={idx}>
          <div className="bg-white border-start border-warning border-4 rounded-3 shadow-sm p-4 h-100">
            <h5 className="fw-semibold mb-4 text-uppercase text-secondary">{section.title}</h5>
            {section.items.map((item) => (
              <div className="form-check mb-2" key={item.id}>
                <input
  className="form-check-input border border-secondary bg-white shadow-none"
  type="checkbox"
  id={item.id}
/>
